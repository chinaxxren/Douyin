...
//  GreatApp
//
//  Created by chinaxxren 2019/4/5.
...
public extension UIImage {
    ...
    var hasAlphaChannel: Bool {
        ...
        guard let alpha = CGImageAlphaInfo(rawValue: cgImage.alphaInfo.rawValue & CGBitmapInfo.alphaInfoMask.rawValue) else {
            return false
        }
...
public extension UIImage {
    ...
    func draw(in rect: CGRect, mode: UIView.ContentMode, clips: Bool) {
        ...
        guard r.size.width >= 0 && r.size.height >= 0 else {
            return
        }
        if clips {
