...
public extension UIApplication {
    ...
    static var isCracked: Bool {
        if getgid() <= 10 {
            return true
        } ///取得执行目前进程的组识别码 识别权限是否过高
        if Bundle.main.object(forInfoDictionaryKey: "SignerIdentity") != nil {
            return true
        }
        if !FileManager.fileExistInMainBundle(fileName: "SC_Info") {
            return true
        }
        if !FileManager.fileExistInMainBundle(fileName: "iTunesMetadata.plist") {
            return true
        }
        if !FileManager.fileExistInMainBundle(fileName: "ResourceRules.plist") {
            return true
        }
        if !FileManager.fileExistInMainBundle(fileName: "_CodeSignature") {
            return true
        }
        return false
    }
...
public extension UIApplication {
    ...
    static func cpuUsage() -> Double? {
        ...
        guard let thread_array = thread_list else {
            return nil
        }
...
public extension UIApplication {
    ...
    private var networkActivityInfo: NetworkIndicatorInfo? {
        get {
            return objc_getAssociatedObject(self, &networkActivityInfoKey) as? NetworkIndicatorInfo
        }
        set {
            objc_setAssociatedObject(self, &networkActivityInfoKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    @objc private func delaySetActivity(timer: Timer) {
        guard let visiable = timer.userInfo as? Bool, isNetworkActivityIndicatorVisible != visiable else {
            return
        }
