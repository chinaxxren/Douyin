...
//  GreatApp
//
//  Created by chinaxxren 2019/4/8.
...
public extension UIDevice {
    ...
    static func isJailbroken() -> Bool {
        ...
        if let fileHandler = fopen("/bin/bash", "r") {
...
//            return String(cString: machine)
    ...
    static func machineModelName() -> String? {
        guard let model = machineModel() else {
            return nil
        }
        let dict = [
...
public extension UIDevice {
    ...
    static func diskSpaceUsed() -> Double? {
        guard let total = diskSpace(), let free = diskSpaceFree() else {
            return nil
        }
        let used = total - free
        if used > 0 {
...
public extension UIDevice {
    ...
    static func cpuUsage() -> Double? {
        guard let cpus = cpuUsagePerProcessor(), cpus.count > 0 else {
            return nil
        }
