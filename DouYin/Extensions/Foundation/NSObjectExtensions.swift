...
//  GreatApp
//
//  Created by chinaxxren 2019/4/11.
...
public extension NSObject {
    private(set) var keyPathDict: NSMutableDictionary? {
        get {
            return objc_getAssociatedObject(self, &keyPathDictKey) as? NSMutableDictionary
        }
        set {
            objc_setAssociatedObject(self, &keyPathDictKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    class KVOClosureTarget: NSObject {
        ...
        override public func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
            ...
            if let isPrior = (c[.notificationIsPriorKey] as? Bool), isPrior {
                return
            }
            if let kind = c[.kindKey] as? NSKeyValueChange, kind == .setting {
                return
            }
...
public extension NSObject {
    ...
    func removeObserverAction(keyPath: String) {
        guard let dict = keyPathDict, let arr = dict.object(forKey: keyPath) as? Array<KVOClosureTarget> else {
            return
        }
        arr.forEach {
            self.removeObserver($0, forKeyPath: keyPath)
        }
    ...
    func removeAllObserverAction() {
        guard let dict = keyPathDict as? [String: Array<KVOClosureTarget>] else {
            return
        }
        dict.forEach { (key, arr) in
            arr.forEach {
                removeObserver($0, forKeyPath: key)
            }
        }
    }
}
