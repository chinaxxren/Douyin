...
//  DonYin
//
//  Created by chinaxxren 2019/5/23.
...
class VideoFeedViewController: UIViewController {
    ...
    required init() {
        tableView = DYTableView(frame: CGRect.zero, style: .plain)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                self.currentObserver = self.currentSubject.asDriver().drive(
                        onNext: { [weak self](index) in
                            guard let `self` = self, let cell = self.tableView.cellForRow(at: IndexPath(row: index, section: 0)) as? VideoFeedCell else {
                                return
                            }
                            if cell.isReadyToPlay {
                                ...
                                cell.startPlayOnReady = { [weak cell, weak self] in
                                    guard let `self` = self, let cell = cell, let indexPath = self.tableView.indexPath(for: cell) else {
                                        return
                                    }
...
extension VideoFeedViewController {
    func addBackgroundImage() {
        let backgroundImage = DYImageView()
...
extension VideoFeedViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        guard let cell = tableView.visibleCells.first, let index = tableView.indexPath(for: cell) else {
            return
        }
        guard currentSubject.value != index.row else {
            return
        }
