...
//  DonYin
//
//  Created by chinaxxren 2019/6/5.
...
class NavigationViewController: UINavigationController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        guard let interactivePopGes = interactivePopGestureRecognizer else {
            return
        }
        guard let interactivePopView = interactivePopGes.view else {
            return
        }

        guard let targets = interactivePopGes.value(forKey: "_targets") as? [NSObject] else {
            return
        }
        guard let target = targets.first?.value(forKey: "target") else {
            return
        }
