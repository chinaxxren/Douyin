...
class VideoFeedCell: DYTableViewCell {
    private var playImageView: DYImageView!
    ...
    private var avatarBtn: DYButton!
    private var followBtn: AnimationView!
    private var musicIcon: DYImageView!
    ...
    public func bind(viewModel: VideoCellViewModel) {
        ...
        viewModel.musicThumb.bind { [weak self] in
            guard let url = $0, let `self` = self else {
                return
            }
...
//            })
    ...
    private func playImageAnimation(status: ZPlayerStatus) {
        ...
        case .pause:
            ...
            self.playImageView.transform = CGAffineTransform(scaleX: 1.8, y: 1.8)
...
extension VideoFeedCell {
    ...
    func addPlayImage() {
        playImageView = DYImageView()
        playImageView.image = UIImage(named: "icon_play_pause52x62")
        contentView.addSubview(playImageView)
        playImageView.translatesAutoresizingMaskIntoConstraints = false
        playImageView.centerXAnchor.constraint(equalTo: contentView.centerXAnchor, constant: 0).isActive = true
        playImageView.centerYAnchor.constraint(equalTo: contentView.centerYAnchor, constant: 0).isActive = true
    }
    ...
    func addAvatarBtn() {
        avatarBtn = DYButton(type: .system)
    ...
    func addFollowBtn() {
        if followBtn != nil, followBtn.superview != nil {
            return
        }
