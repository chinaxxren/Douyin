...
extension VideoCellViewModel {
    ...
    public var diggCount: Driver<String> {
        return awemeDriver.map {
            $0.statistics.diggCount.readability
        }.asDriver()
    }

    public var commentCount: Driver<String> {
        return awemeDriver.map {
            $0.statistics.commentCount.readability
        }.asDriver()
    }

    public var shareCount: Driver<String> {
        return awemeDriver.map {
            $0.statistics.shareCount.readability
        }.asDriver()
    }

    public var musicThumb: Observable<URL?> {
        return awemeDriver.map {
            guard let str = $0.music?.coverThumb.urlList.first else {
                return nil
            }
    ...
    public var avatarThumb: Observable<URL?> {
        return awemeDriver.map {
            guard let str = $0.author.avatarThumb.urlList.first else {
                return nil
            }
            return URL(string: str) ?? nil
        }.asObservable()
    ...
    public var playUrl: Observable<URL?> {
        return awemeObserver.map { (aweme) -> URL? in
            guard let urlStr = aweme.video.playAddr.urlList.first else {
                return nil
            }
    ...
    public var musicName: Driver<String> {
        return awemeDriver.map { (aweme) -> String in
            guard let title = aweme.music?.title else {
                return ""
            }
    ...
    public var dynamicCover: Driver<URL?> {
        return awemeDriver.map { (aweme) -> URL? in
            guard let urlStr = aweme.video.dynamicCover.urlList.first else {
                return nil
            }
            let url = URL(string: urlStr)
            return url
    ...
    public var topIcon: Driver<URL?> {
        return awemeDriver.map { (aweme) -> URL? in
            guard let urlStr = aweme.labelTop?.urlList.first else {
                return nil
            }
            let url = URL(string: urlStr)
            return url
