...
public class Image: UIImage {
    ...
    convenience init?(named name: String) {
        guard name.isNotBlank, !name.hasPrefix("/") else {
            return nil
        }
        ...
        outer: for s in scales {
            ...
            for ext in exts {
                guard let p = Bundle.main.path(forResource: imgName, ofType: ext) else {
                    continue
                }
                path = p
                break outer
    ...
    override convenience init?(data: Data, scale: CGFloat) {
        guard let decoder = Decoder.decode(for: data, scale: scale),
              let image = decoder.frame(at: 0, decodeForDisplay: true) else {
            return nil
        }
