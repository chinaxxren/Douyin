...
extension Wrapper where Base: UIButton {
    ...
    private func setTaskIdentifier(_ identifier: Identifier.Value?, for state: UIControl.State) {
        taskIdentifierInfo[NSNumber(value: state.rawValue)] = identifier.map {
            Box($0)
        }
    }
    ...
    private var imageTask: DownloadTask? {
        ...
        set {
            objc_setAssociatedObject(base, &imageTaskKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
}
...
extension Wrapper where Base: UIButton {
    ...
    private func setBackgroundTaskIdentifier(_ identifier: Identifier.Value?, for state: UIControl.State) {
        backgroundTaskIdentifierInfo[NSNumber(value: state.rawValue)] = identifier.map {
            Box($0)
        }
    }
    ...
    private var backgroundImageTask: DownloadTask? {
        ...
        set {
            objc_setAssociatedObject(base, &backgroundImageTaskKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
}
