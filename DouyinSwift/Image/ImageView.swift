...
open class ImageView: UIImageView {
    ...
    public var runloopMode = RunLoop.Mode.common {
        willSet {
            guard newValue == runloopMode else {
                return
            }
            stopAnimating()
    ...
    class Animator {
        ...
        private func setupAnimatedFrames() {
            ...
            (0..<frameCount).forEach { (index) in
                ...
                if self.frameCacheMode == .memoryPreferred, index > framePreloadCount {
                    return
                }
        ...
        private func loadFrame(at index: Int) -> CGImage? {
            ...
            if needsPrescaling, size != .zero, imageSize.width > size.width || imageSize.height > size.height {
                guard let imageRef = animationImage.frame(at: index, decodeForDisplay: false) else {
                    return nil
                }
