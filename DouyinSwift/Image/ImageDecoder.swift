...
class Decoder {
    ...
    private func _update(data: Data, final: Bool) -> Bool {
        ...
        if data.count < self.data?.count ?? 0 {
            return false
        }
        finalized = final
        self.data = data
    ...
    private func _updateSourceImageIO() {
        ...
        (0..<frameCount).forEach { (index) in
            ...
            if let properties = CGImageSourceCopyPropertiesAtIndex(source, index, nil) as? [String: Any] {
                ...
                if let widthNum = properties[kCGImagePropertyPixelWidth as String] as? Int {
                    frame.width = widthNum
                }
                if let heightNum = properties[kCGImagePropertyPixelHeight as String] as? Int {
    ...
    private func _frame(at index: Int, decodeForDisplay: Bool) -> CGImage? {
        ...
        if !needBlend {
            guard var imageRef = _newUnblendedImage(at: index, extendToCanvas: extendToCanvas, decoded: &decoded) else {
                return nil
            }
        ...
        guard _createBlendContextIfNeeded() else {
            return nil
        }
    ...
    private func _newUnblendedImage(at index: Int, extendToCanvas: Bool, decoded: UnsafeMutablePointer<Bool>?) -> CGImage? {
        ...
        if let source = source {
            guard var imageRef = CGImageSourceCreateImageAtIndex(source, index, [kCGImageSourceShouldCache: true] as CFDictionary) else {
                return nil
            }
            if extendToCanvas {
        ...
        if let webpSource = webpSource {
            let iter = UnsafeMutablePointer<WebPIterator>.allocate(capacity: 1)
            //WebP帧索引从1开始
            guard WebPDemuxGetFrame(webpSource, Int32(index + 1), iter) != 0 else {
                return nil
            }

            let frameWidth = Int(iter.pointee.width)
            let frameHeight = Int(iter.pointee.height)
            guard frameWidth >= 1 && frameHeight >= 1 else {
                return nil
            }
            ...
            guard let provider = CGDataProvider(dataInfo: pixels, data: pixels, size: length, releaseData: { info, _, _ in if let info = info {
                info.deallocate()
            } }) else {
                pixels.deinitialize(count: length)
                pixels.deallocate()
    ...
    func _blendImage(with frame: ImageDecoderFrame) {
        if frame.dispose == .previous {
            ...
            if frame.blend == .over {
                guard let unblendImage = _newUnblendedImage(at: frame.index, extendToCanvas: false, decoded: nil) else {
                    return
                }
    ...
    private func _ImageCreateDecodedCopy(imageRef: CGImage, decodeForDisplay: Bool) -> CGImage? {
        ...
        if decodeForDisplay {
            ...
            guard let context = CGContext(data: nil, width: width, height: height, bitsPerComponent: 8, bytesPerRow: 0, space: CGColorSpaceCreateDeviceRGB(), bitmapInfo: bitmapInfo.rawValue | mask.rawValue) else {
                return nil
            }
            ...
            guard let space = colorSpace, bytesPerRow != 0 && width != 0 && height != 0 else {
                return nil
            }
            guard let dataProvider = imageRef.dataProvider, let data = dataProvider.data else {
                return nil
            }
            guard let newProvider = CGDataProvider(data: data) else {
                return nil
            }
