...
public extension UIFont {
    ...
    /// 返回当前字体的粗体版本
    var bold: UIFont {
        guard let boldDesc = fontDescriptor.withSymbolicTraits(.traitBold) else {
            return self
        }
    ...
    /// 返回当前字体的斜体版本
    var italic: UIFont {
        guard let italicDesc = fontDescriptor.withSymbolicTraits(.traitItalic) else {
            return self
        }
    ...
    var boldItalic: UIFont {
        guard let boldItalizDesc = fontDescriptor.withSymbolicTraits([.traitBold, .traitItalic]) else {
            return self
        }
    ...
    /// 返回当前字体的普通版本
    var normal: UIFont {
        guard let normalDesc = fontDescriptor.withSymbolicTraits(.init()) else {
            return self
        }
...
public extension UIFont {
    ...
    static func unloadFont(font: UIFont) -> Bool {
        guard let cgFont = CGFont(font.fontName as CFString) else {
            return false
        }
        var error: Unmanaged<CFError>? = nil
        defer {
    ...
    static func data(font: UIFont) -> Data? {
        guard let cgFont = font.CGFontRef else {
            return nil
        }
        guard let tags = cgFont.tableTags else {
            return nil
        }
        ...
        var offsetTable: FontHeader = dataPtr.withMemoryRebound(to: FontHeader.self, capacity: 1) {
            $0.pointee
        }
        var cStr = "OTTO".cString(using: .utf8)!
        let otto = UnsafeMutablePointer<CChar>(&cStr).withMemoryRebound(to: Int32.self, capacity: 1) {
            $0.pointee
        }
        ...
        var entryPtr = dataPtr.withMemoryRebound(to: TableEntry.self, capacity: 1) {
            $0
        }
        ...
        (0..<tableCount).forEach { (index) in
            if let aTag: UInt32 = CFArrayGetValueAtIndex(tags, index)?.assumingMemoryBound(to: UInt32.self).pointee, let tableData = cgFont.table(for: aTag) {
                ...
                entry.fTag = aTag
                let table = dataPtr.withMemoryRebound(to: TableEntry.self, capacity: 1) {
                    $0
                }
